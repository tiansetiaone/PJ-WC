...
function decodeParamsV2ByABI(funABI, data) {
    ...
    const convertAddresses = (addrArr) => {
        if (Array.isArray(addrArr)) {
            ...
            return (0, address_js_1.toHex)(addrArr);
        }
    };
    const mapTuple = (components, args, dimension) => {
        if (dimension > 1) {
    ...
    const setResultProp = (result, name, value) => {
    ...
    const decodeResult = (outputs, result) => {
        if (outputs.length)
            outputs.forEach((output, i) => {
                ...
                if (result[i]) {
                    if (type === 'address') {
                        result[i] = (0, address_js_1.toHex)(result[i]);
                        setResultProp(result, name, (0, address_js_1.toHex)(result[i]));
                    }
                    else if (type.match(/^([^\x5b]*)(\x5b|$)/)[0] === 'address[') {
